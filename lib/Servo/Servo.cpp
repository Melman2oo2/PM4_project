...
float Servo::getCurrentVelocity(){
    return m_Motion.velocity;
}
...
// getMaxAcceleration implementiert durch Akermleo

float Servo::getMaxAcceleration(){
    return (m_Motion.getProfileAcceleration())/(m_pulse_max - m_pulse_min);
}
...
// getCurrentPulseWith implementiert durch Akermleo

float Servo::getCurrentPulseWidth(){
    float pos = static_cast<float>(m_Motion.position);
    return ((pos - m_pulse_min)/(m_pulse_max - m_pulse_min));
}

// getTargetPulseWith implementiert durch Akermleo

float Servo::getTargetPulseWith(){
    return ((m_pulse - m_pulse_min)/(m_pulse_max - m_pulse_min));
}
